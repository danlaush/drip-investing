...
export const mutations = {
  ...
  insertIntoIndex (state, record) {
    const date = moment(record.date)
    const y = date.year()
    const m = date.month() + 1
    const d = date.date()
    state.index[y] = state.index[y] || {}
    state.index[y][m] = state.index[y][m] || {}
    state.index[y][m][d] = state.index[y][m][d] || {}
    state.index[y][m][d][record.id] = record
  }
}

export const actions = {
  async nuxtServerInit ({commit}, {app}) {
    // let {data} = await app.$axios.get('/data/data_short.json')
    // commit('setEmployees', values(data))
    // console.log(data);
    commit('setItems', data)
    data.map(record => commit('insertIntoIndex', record))
  }
}